...
- [Helm chart / 定期审计 CronJob](docs/helm-chart.md)
- [Redaction / 脱敏](docs/redaction.md)
- [Public demo case study](docs/public-demo-case-study.md)
...
## Generate a delivery bundle

`bundle` creates a delivery directory and zip archive containing HTML,
Markdown, JSON, a manifest, and a short README. When using a live Prometheus or
Grafana endpoint, it also includes `doctor` output unless disabled.
...
The Helm chart installs a read-only CronJob that runs `ai-gpu-lens bundle` on a
schedule and writes report files to a PVC. It does not need Kubernetes API write
permissions.
...
For Grafana datasource proxy credentials, use a Secret and a values file. See
[Helm chart](docs/helm-chart.md).

## Redact reports for public demos

`redact` replaces environment identifiers with deterministic aliases while
keeping utilization, capacity, and cost numbers intact.
...
See [Redaction](docs/redaction.md) and the
[public demo case study](docs/public-demo-case-study.md).
...
- Object storage export for scheduled audit bundles
